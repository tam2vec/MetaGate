...
This page separates what MetaGate already demonstrates from what remains
production work.

## One-command proof bundle

Run `./scripts/judge_proof.sh` from the repository root. It produces
`/tmp/metagate-release-proof.json` with the exact commit and the status of:
...
- agreement between MetaGate's Skill and MCP surfaces;
...
| Claim | Evidence |
| --- | --- |
| MetaGate can evaluate DataHub metadata for a specific AI action. | `src/context_gradient/datahub/adapter.py`, live GraphQL mode, and `examples/outputs/live-runs.json` |
| MetaGate produces both allowed and blocked decisions. | `examples/outputs/allowed-action.json`, `examples/outputs/blocked-action.json`, and the visual review page |
| Decisions are explainable. | `examples/outputs/explainability-report.json`, `examples/outputs/action-metagate.json` |
| The same engine supports human UI and machine JSON. | `examples/outputs/metagate-demo-app.html`, CLI output, context contract JSON |
| A DataHub page can trigger MetaGate automatically. | `examples/browser-extension/` reads the DataHub asset URN and calls `/api/evaluate`. |
| DataHub can remain the system of record. | `examples/datahub-embed/` panel prototype and `docs/why-metagate.md` |
...
| --- | --- |
| The DataHub panel is not packaged as a production DataHub plugin. | `dist/MetaGate-DataHub-extension.zip` is installable in Chrome; native DataHub frontend registration remains deployment-specific. |
...
| Live write-back depends on DataHub version and custom mutation support. | MetaGate is read-only by default and requires an approved mutation, authorized token, and read-back verification before writing. |
...
2. Show the browser extension panel auto-running MetaGate from the asset URL.
3. Run MetaGate against the same DataHub asset URN in the terminal as backup proof.
...
> MetaGate adds a deterministic AI action gate to DataHub: it reads metadata
> evidence, checks an action metagate, and returns an explainable allow/block
