...
MetaGate cannot safely ship a universal DataHub write-back mutation because
...
1. Configure a non-production DataHub deployment with a tested mutation for a
   MetaGate Certificate custom aspect or equivalent metadata field.
2. Set `DATAHUB_CERTIFICATE_MUTATION`.
3. Run MetaGate on the same asset URN.
4. Reopen the DataHub asset page.
5. Capture a screenshot showing the MetaGate Certificate, custom property,
...
- the MetaGate Review UI showing the certificate and remediation path
...
3. MetaGate Review write-back queue
...
- visible MetaGate result or metadata field
...
> MetaGate is read-only by default. This receipt shows the certificate and task
