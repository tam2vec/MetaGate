...
| --- | --- | --- |
| Packaged DataHub plugin | Browser extension prototype, DataHub panel contract, automatic URN-detection flow, screenshot checklist, and Context Gradient Preflight action contract exist. A production DataHub package is still future work. | "The automatic UX is proven through a browser extension prototype; DataHub packaging is the next integration step." |
...
| Independent benchmark | CSV template, scorer, protocol, reviewer request, and unsafe-answer reduction benchmark framing are included. External labels are still needed from reviewers. | "Curated benchmark passes; independent agreement and unsafe-answer reduction can be computed once reviewers label cases." |
