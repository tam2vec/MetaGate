...
| Independent benchmark | The 30-case curated conformance suite, blank label template, scorer, protocol, reviewer request, and unsafe-answer reduction framing are included. The 10 informal sanity labels are not independent held-out evidence. | "Predicate passes its curated checks; independent human validation is prepared but not yet completed." |
...
| Install friction | Package entrypoints now include `predicate`, `predicate-review`, `predicate-mcp`, and `predicate-doctor`; `scripts/verify_predicate.sh` runs tests, rebuilds the extension ZIP, and checks prerequisites. | "The demo can be installed, checked, and run with explicit commands." |
