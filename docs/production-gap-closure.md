...
| --- | --- | --- |
| Packaged DataHub plugin | Browser extension prototype, DataHub panel contract, automatic URN-detection flow, and screenshot checklist exist. A production DataHub package is still future work. | "The automatic UX is proven through a browser extension prototype; DataHub packaging is the next integration step." |
...
| Live write-back screenshot | Safe payloads, receipt command, receipt file, UI write-back queue, and mutation-gated adapter exist. A real DataHub UI mutation still needs a deployment-supported mutation. | "Read-only by default; write-back is explicit and deployment-owned." |
| Independent benchmark | CSV template, scorer, protocol, and reviewer request are included. External labels are still needed from reviewers. | "Curated benchmark passes; independent agreement can be computed once reviewers label cases." |
...
| Security story | Security policy, write-back safety docs, and UI Security & RBAC panel exist. Production RBAC/token controls must be configured by deployment. | "Predicate can be read-only, uses DataHub auth token server-side, and gates mutation write-back." |
