(function () {
  ...
  const PANEL_ID = "metagate-datahub-auto-panel";
  ...
  function failedTerms(run) {
    const metagate = run.metagate || run.action_metagate || {};
    return run.failed || metagate.failed_terms || [];
  }
  ...
  function remediationFor(run) {
    ...
    const assetMaps = {
      fct_users_created: {
        ...
        "confidence >= 88.0": "Use DataHub assertion run history as evidence, then rerun MetaGate on fct_users_created."
      },
      fct_users_deleted: {
    ...
    const fallback = {
      ...
      "readiness_score >= 92.0": "Repair high-severity metadata gaps, then rerun MetaGate.",
  ...
  function buildPanel(urn) {
    ...
    panel.id = PANEL_ID;
    panel.innerHTML = `
      <div class="metagate-card">
        <div class="metagate-heading">
          <div>
            <div class="metagate-kicker">MetaGate</div>
            <h2>AI action check</h2>
          </div>
          <button class="metagate-close" type="button" aria-label="Close MetaGate panel" title="Close MetaGate panel">&times;</button>
        </div>
        <div class="metagate-status">Evaluating DataHub metadata...</div>
        <div class="metagate-body"></div>
      </div>
    `;
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .metagate-kicker {
        color: #617086;
        font-size: 12px;
        text-transform: uppercase;
        font-weight: 800;
      }
      #${PANEL_ID} .metagate-heading {
        display: flex;
      ...
      #${PANEL_ID} .metagate-close:hover {
        background: #f5f7fb;
        color: #172033;
      }
      #${PANEL_ID} .metagate-status {
        border: 1px solid #dce3ee;
        border-radius: 8px;
        background: #f5f7fb;
        padding: 12px;
      ...
      #${PANEL_ID} .metagate-decision > span {
        display: block;
        min-width: 0;
        max-width: 100%;
      ...
      #${PANEL_ID} .metagate-decision strong {
        display: block;
        font-size: 30px;
        text-transform: uppercase;
        margin: 3px 0;
      ...
      #${PANEL_ID} .metagate-meta {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 8px;
        margin: 10px 0;
      }
      #${PANEL_ID} .metagate-metric {
        border: 1px solid #dce3ee;
        border-radius: 8px;
        padding: 10px;
        background: #f5f7fb;
      }
      #${PANEL_ID} .metagate-metric span {
        display: block;
        color: #617086;
        font-size: 12px;
      }
      #${PANEL_ID} .metagate-metric strong {
        display: inline-flex;
        align-items: center;
        border-radius: 999px;
        padding: 3px 9px;
      ...
      #${PANEL_ID} .metagate-metric strong.strong {
        background: #e8f7ee;
        color: #13824c;
      }
      #${PANEL_ID} .metagate-metric strong.warn {
        background: #fff6df;
        color: #a96d10;
      }
      #${PANEL_ID} .metagate-metric strong.weak {
        background: #fff0f0;
        color: #bc3030;
      }
      #${PANEL_ID} ol {
    ...
    panel.querySelector(".metagate-close").addEventListener("click", () => {
      panel.dataset.dismissed = "true";
      panel.remove();
      window.__metagateDismissedUrl = location.href;
    });
    return panel;
  }
  ...
  function renderResult(panel, run, base) {
    ...
    body.innerHTML = `
      ...
      <div class="metagate-meta">
        <div class="metagate-metric"><span>Readiness</span><strong class="${metricClass(readiness)}">${readiness}</strong></div>
  ...
  async function evaluateCurrentAsset(force = false) {
    ...
    if (!urn) {
      ...
      window.__metagateDismissedUrl = null;
      return;
    }
    if (window.__metagateDismissedUrl === location.href) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`MetaGate API returned ${response.status}: ${detail.slice(0, 160)}`);
      }
      renderResult(panel, await response.json(), base);
    } catch (error) {
      renderError(panel, `MetaGate could not evaluate this asset. ${error.message || "Check the configured API URL."}`);
    }
  }
