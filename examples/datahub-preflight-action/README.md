...
MetaGate Preflight is the intended DataHub-native gate: it checks whether a
...
3. DataHub sends the dataset URN and requested AI capability to MetaGate.
4. MetaGate evaluates metadata readiness.
...
`DatasetProperties.customProperties` under `metagate.ai_context_contract`,
...
`examples/browser-extension`, plus the MCP server exposed by `metagate-mcp`.
...
> MetaGate Preflight is the native DataHub action contract. The hackathon
