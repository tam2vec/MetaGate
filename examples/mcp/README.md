# MetaGate MCP integration

MetaGate exposes three MCP tools over stdio:

- `metagate_evaluate`: return allow/block, scores, failed terms, and evidence
- `metagate_constraint_contract`: return the agent boundary: allowed action,
  forbidden actions, required approval, permitted scope, and decision ID
- `metagate_evidence`: return the evidence facts and gaps behind a decision

This lets an agent call MetaGate directly instead of running a shell command.
...
For an MCP client, copy the settings in `metagate-mcp.json` and change the
...
JSON text into MetaGate evidence: owner, glossary, lineage, schema/column
coverage, latest assertion result, freshness, incidents, usage, and policy.
When the server exposes the optional `get_dataset_queries` tool, MetaGate
