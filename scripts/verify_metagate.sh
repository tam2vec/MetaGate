#!/bin/sh
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
PROJECT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
cd "$PROJECT_DIR"

printf '%s\n' '1/3 Running repository tests...'
PYTHONPYCACHEPREFIX="${PYTHONPYCACHEPREFIX:-/tmp/metagate-pycache}" \
  PYTHONPATH=src python3 -m unittest discover -s tests -q

printf '%s\n' '2/3 Building the browser extension package...'
./scripts/package_extension.sh >/tmp/metagate-package.log
tail -n 1 /tmp/metagate-package.log

printf '%s\n' '3/3 Running the deterministic enforcement proof...'
PYTHONPYCACHEPREFIX="${PYTHONPYCACHEPREFIX:-/tmp/metagate-pycache}" \
  PYTHONPATH=src python3 scripts/run_enforcement_demo.py \
    --datahub-file examples/data/six_asset_review_graph.json \
    >/tmp/metagate-enforcement-proof.json
PYTHONPYCACHEPREFIX="${PYTHONPYCACHEPREFIX:-/tmp/metagate-pycache}" \
  python3 - <<'PY'
import json
from pathlib import Path

proof = json.loads(Path('/tmp/metagate-enforcement-proof.json').read_text())
decisions = {item['action']: item['decision'] for item in proof['decisions']}
expected = {
    'answer-business-questions': 'allowed',
    'generate-executive-metrics': 'blocked',
    'modify-dataset': 'blocked',
    'restricted-sql': 'blocked',
}
if decisions != expected:
    raise SystemExit(f'Unexpected enforcement proof: {decisions!r}')
if proof['integration_proof']['status'] != 'verified':
    raise SystemExit('Skill/MCP integration proof did not agree.')
print('Enforcement story, constraint contract, Skill, and MCP proof passed.')
PY

printf '%s\n' 'Optional local environment status:'
doctor_status=0
doctor_output=$(PYTHONPATH=src python3 -m metagate.doctor 2>/tmp/metagate-doctor-error.log) || doctor_status=$?
if [ "$doctor_status" -eq 0 ]; then
  printf '%s\n' 'Required local checks passed. Official DataHub MCP is informational unless configured.'
else
  printf '%s\n' 'Required local environment is unavailable; deterministic repository proof still passed.'
  cat /tmp/metagate-doctor-error.log >&2 || true
fi
printf '%s\n' "$doctor_output"

printf '%s\n' 'MetaGate verification passed.'
