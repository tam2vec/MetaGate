...
def _assertion_result_state(rows: list[dict[str, Any]]) -> str:
    ...
    if any(value in {"FAIL", "FAILURE", "ERROR", "FAILED", "UNKNOWN"} for value in statuses):
        return "failing"
    if all(value in {"PASS", "PASSED", "SUCCESS", "SUCCEEDED", "OK", "COMPLETE"} for value in statuses):
        return "passing"
    return "unknown"
...
def _compact_evidence(run: dict[str, Any]) -> dict[str, Any]:
    ...
    usage_row = _row(rows, "usage")
    policy_row = _row(rows, "policy")
    return {
        "assertions": {
        ...
        "usage": {
            "status": _evidence_status("usage", usage_row, facts),
            "query_count": usage.get("query_count", usage.get("queries")),
            "last_queried_at": usage.get("last_queried_at", usage.get("timestamp")),
            "observed_at": usage_row.get("observed_at"),
        },
        "policy": {
            "status": _evidence_status("policy", policy_row, facts),
            "profile": policy.get("profile", policy.get("name")),
            "observed_at": policy_row.get("observed_at"),
        },
    }
...
def build_constraint_contract(run: dict[str, Any], capability: str | None = None) -> dict[str, Any]:
    ...
    failed_terms = run.get("failed_terms") or run.get("failed") or []
    if not isinstance(failed_terms, list):
        failed_terms = [str(failed_terms)]
    ...
    registry = run.get("agent_context") or run.get("registry_evidence") or {
        "status": "not_requested",
        "source": "agent-registry-not-configured",
        "blocking_reasons": [],
        "evidence": [],
    }
    capability_records = run.get("certified_capabilities") or run.get("capabilities") or []
    selected_capability = next(
        (item for item in capability_records if isinstance(item, dict) and item.get("capability") == capability),
        {},
    )
    required_evidence = selected_capability.get("required_evidence") or run.get("required_evidence") or []
    evidence_status = selected_capability.get("evidence_status") or run.get("evidence_status") or {}
    if not evidence_status:
        evidence_status = {
            str(kind): str(item.get("status"))
            for kind, item in (coverage.get("items") or {}).items()
            if isinstance(item, dict) and item.get("status")
        }
    required_evidence = sorted({str(item) for item in required_evidence})
    evidence_status = {str(key): str(value) for key, value in evidence_status.items()}
    ...
    return {
        ...
        "failed_terms": _dedupe([str(value) for value in failed_terms]),
        ...
        "incident_investigation": run.get("incident_investigation") or {},
        "mcp_trace": run.get("mcp_trace") or [],
        "official_datahub_mcp": run.get("official_datahub_mcp") or {
            "status": "not_configured",
            "server": "DataHub official MCP server",
        },
        "mcp_gate": run.get("mcp_gate") or {
            "required": False,
            "status": "not_configured",
            "decision_effect": "informational",
        },
        "registry_required": bool(run.get("registry_required")),
        "registry_evidence": registry,
        "agent_context": {
            "agent_urn": registry.get("agent_urn"),
            "skill_urn": registry.get("skill_urn"),
            "tool_urn": registry.get("tool_urn"),
            "service_urn": registry.get("service_urn"),
            "status": registry.get("status", "not_requested"),
        },
        "tool_boundary": {
            "enforced_by": "metagate.agent_gate.guarded_tool_call",
            "denial_mode": "raises ToolCallDenied before the callable is invoked",
        },
    }
