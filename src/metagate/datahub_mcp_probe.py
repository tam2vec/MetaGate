...
class DataHubMCPProbe:
    def __init__(self, command: str | None = None, *, timeout_seconds: float = DEFAULT_TIMEOUT_SECONDS) -> None:
        self.command = command if command is not None else os.environ.get("METAGATE_DATAHUB_MCP_COMMAND", "")
        self.timeout_seconds = max(1.0, float(timeout_seconds))
        self._request_ids = count(1)
        # The official DataHub MCP server uses newline-delimited JSON over
        # stdio. Keep Content-Length framing for older/local test servers.
        configured_framing = os.environ.get("METAGATE_MCP_FRAMING", "").strip().lower()
        self._newline_framing = configured_framing == "newline" or "mcp-server-datahub" in self.command
    ...
    def run(self, urn: str) -> dict[str, Any]:
        ...
        if not self.command.strip():
            return {
                ...
                "trace": trace,
                "note": "Set METAGATE_DATAHUB_MCP_COMMAND to the approved official DataHub MCP startup command to run this proof.",
            }
        if not urn.strip():
        ...
        try:
            ...
            if not command:
                raise MCPProbeError("METAGATE_DATAHUB_MCP_COMMAND is empty")
            ...
            trace.append({"method": "initialize", "status": "requested"})
            initialize = self._request(process, "initialize", {
                "protocolVersion": "2024-11-05",
                "capabilities": {},
                "clientInfo": {"name": "metagate", "version": "0.1.0"},
            })
            trace[-1]["status"] = "completed"
            self._notification(process, "notifications/initialized", {})
            trace.append({"method": "tools/list", "status": "requested"})
            ...
            if arguments is None:
                ...
                trace.append({"method": "tools/call:get_entities", "status": "requested", "argument_shape": sorted(arguments)})
            ...
            if query_arguments is not None:
                trace.append({"method": "tools/call:get_dataset_queries", "status": "requested", "argument_shape": sorted(query_arguments)})
    ...
    def _write_frame(self, process: subprocess.Popen[bytes], payload: dict[str, Any]) -> None:
        if process.stdin is None:
            raise MCPProbeError("MCP process stdin is unavailable")
        encoded = json.dumps(payload, separators=(",", ":")).encode("utf-8")
        if self._newline_framing:
    ...
    def _read_frame(self, process: subprocess.Popen[bytes], deadline: float) -> dict[str, Any] | None:
        ...
        if self._newline_framing:
            return self._read_newline_frame(process, deadline)
    ...
    def _read_newline_frame(self, process: subprocess.Popen[bytes], deadline: float) -> dict[str, Any] | None:
        """Read one newline-delimited JSON-RPC message from an MCP stdio server."""
        if process.stdout is None:
            raise MCPProbeError("MCP process stdout is unavailable")
        selector = selectors.DefaultSelector()
        selector.register(process.stdout, selectors.EVENT_READ)
        try:
            while True:
                ...
                try:
                    ...
                    raise MCPProbeError(f"MCP returned invalid JSON: {error}") from error
                return value if isinstance(value, dict) else None
        finally:
            selector.close()
...
def main() -> None:
    ...
    parser.add_argument("--command", default=os.environ.get("METAGATE_DATAHUB_MCP_COMMAND"))
