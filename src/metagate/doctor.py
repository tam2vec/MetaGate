"""Diagnose the local MetaGate and DataHub integration."""
...
def optional_mcp_status() -> tuple[bool, str]:
    ...
    checking that MetaGate's own MCP server file exists. It is therefore an
    informational check: absent configuration is not a local setup failure.
    """
    command = os.environ.get("METAGATE_DATAHUB_MCP_COMMAND", "").strip()
    if command:
        return True, f"configured; run metagate-datahub-mcp-probe to verify ({command})"
    return True, "optional / not configured; set METAGATE_DATAHUB_MCP_COMMAND to verify it"


def main() -> None:
    parser = argparse.ArgumentParser(description="Check MetaGate and DataHub setup")
    parser.add_argument(
        "--datahub-url",
        default=os.environ.get("DATAHUB_GRAPHQL_URL", "http://localhost:8080/api/graphql"),
    )
    parser.add_argument("--metagate-url", default="http://127.0.0.1:8765")
    args = parser.parse_args()

    checks = []
    for name, url, method in (
        ("DataHub GraphQL", args.datahub_url, "POST"),
        ("MetaGate Review API", f"{args.metagate_url}/healthz", "GET"),
    ):
    ...
    checks.extend(
        [
            ...
            },
            {
                "name": "MCP server",
                "required": True,
                "ok": (ROOT / "src/metagate/mcp_server.py").exists(),
                "detail": "MetaGate MCP (local read-only gate)",
            },
            {
