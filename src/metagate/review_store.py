...
class ReviewStore:
        ...
        with self._connect() as connection:
            connection.executescript(
                ...
                CREATE INDEX IF NOT EXISTS audit_events_lookup
                    ON audit_events (urn, decision_id, id ASC);
                """
            )
    ...
    def add_audit_event(self, record: dict[str, Any]) -> dict[str, Any]:
        """Append one repair/enforcement event to the durable audit trail."""
        payload = record.get("payload") or {}
        with self._connect() as connection:
            connection.execute(
                """INSERT INTO audit_events
                (urn, decision_id, event_type, payload_json, created_at)
                VALUES (?, ?, ?, ?, ?)""",
                (
                    record.get("urn", ""),
    ...
    def audit_events(self, urn: str, decision_id: str | None = None, limit: int = 100) -> list[dict[str, Any]]:
        query = "SELECT urn, decision_id, event_type, payload_json, created_at FROM audit_events WHERE urn = ?"
        parameters: list[Any] = [urn]
        if decision_id:
